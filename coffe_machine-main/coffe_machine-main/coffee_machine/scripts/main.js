// Допустимые номиналы для налички (можно потом будет расширить для безналичной оплаты)
const validCoins = [1, 2, 5, 10]; // Монеты
const validBills = [10, 20, 25, 50, 100, 200]; // Купюры
let balance = 0;

function loadBalance() {
    const result = query('SELECT amount FROM Transactions WHERE type = "addmoney"');
    balance = result.reduce((sum, [amount]) => sum + amount, 0) - 
              query('SELECT amount FROM Transactions WHERE type = "change"').reduce((sum, [amount]) => sum + amount, 0);
    return balance;
}
...
function processCommand(command, print) {
    ...
    switch (action) {
        ...
        case 'addmoney':
            ...
            balance = loadBalance() + amount; // Обновляем баланс
        ...
        case 'choosecoffee':
            const coffeeId = parseInt(args[0]); // Преобразуем в число
            if (isNaN(coffeeId)) {
        ...
        case 'addincoffee':
            ...
            if (ingredient === 'sugar' || ingredient === 'milk') {
                ...
                currentOrder.price += qty * (ingredient === 'sugar' ? 1 : 2); // Цена за добавку
        ...
        case 'executeorder':
            ...
            balance = loadBalance(); // Загружаем актуальный баланс
